<script setup lang="ts">
const motionConfig = {
  initial: { y: 100, opacity: 0 },
  enter: { y: 40, opacity: 1, transition: { duration: 800 } },
  variants: {
    scrolled: {
      y: -40,
      transition: {
        type: "spring",
        stiffness: 50,
        damping: 20,
      },
    },
  },
};
</script>

<template>
  <div
    class="relative bg-white z-20 w-full bg-cover py-20 bg-center flex flex-col items-center"
  >
    <div
      class="relative container w-full bg-[#004d42] rounded-[32px] py-12 lg:!px-[70px] overflow-hidden flex flex-col md:flex-row items-center justify-between"
    >
      <div class="z-10 max-w-lg">
        <h2 class="text-3xl md:text-5xl font-extrabold text-white leading-[1.46] mb-9">
          We are building financial foundations
        </h2>
        <button
          class="flex items-center justify-center gap-2 bg-[#b2eda1] text-[#111] w-[170px] h-[60px] rounded-full font-bold capitalize hover:bg-white transition-colors"
        >
          Let's Talk
          <IconArrowRight class="w-4 h-4" />
        </button>
      </div>

      <div class="relative w-full md:w-1/2 h-64 md:h-auto flex justify-end items-end">
        <div
          v-motion
          :initial="motionConfig.initial"
          :enter="motionConfig.enter"
          :variants="motionConfig.variants"
          class="relative z-0"
        >
          <svg
            width="372"
            height="250"
            viewBox="0 0 372 250"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            class="w-full h-auto scale-125 translate-x-10 translate-y-10"
          >
            <path
              d="M120.81 137.65L15.7733 235.304L0 249.943L46.5404 250L58.3819 235.35L137.134 137.65H120.81Z"
              fill="#95A196"
            />
            <path
              d="M149.295 137.65L90.2023 235.304L81.3184 249.943L127.882 250L132.822 235.35L165.618 137.65H149.295Z"
              fill="#85A088"
            />
            <path
              d="M177.781 137.65L164.622 235.304L162.627 249.943L209.225 250L207.276 235.35L194.105 137.65H177.781Z"
              fill="#5C9F63"
            />
            <path
              d="M206.268 137.65L239.052 235.304L243.947 249.943L290.579 250L281.73 235.35L222.591 137.65H206.268Z"
              fill="#439F4D"
            />
            <path
              d="M234.754 137.65L313.471 235.304L325.267 249.943L371.922 250L356.172 235.35L251.077 137.65H234.754Z"
              fill="#019D12"
            />
            <path d="M137.134 105.312H120.811V137.65H137.134V105.312Z" fill="#22675D" />
            <path d="M165.62 86.6959H149.297V137.65H165.62V86.6959Z" fill="#30766C" />
            <path d="M194.105 66.1309H177.781V137.65H194.105V66.1309Z" fill="#3A8A7F" />
            <path d="M222.591 45.1879H206.268V137.65H222.591V45.1879Z" fill="#6CA88B" />
            <path d="M251.077 23.8549H234.754V137.65H251.077V23.8549Z" fill="#44C486" />
            <path
              d="M120.81 137.65L15.7733 235.304L0 249.943L46.5404 250L58.3819 235.35L137.134 137.65H120.81Z"
              fill="white"
            />
            <path
              d="M149.295 137.65L90.2023 235.304L81.3184 249.943L127.882 250L132.822 235.35L165.618 137.65H149.295Z"
              fill="white"
            />
            <path
              d="M177.781 137.65L164.622 235.304L162.627 249.943L209.225 250L207.276 235.35L194.105 137.65H177.781Z"
              fill="white"
            />
            <path
              d="M206.268 137.65L239.052 235.304L243.947 249.943L290.579 250L281.73 235.35L222.591 137.65H206.268Z"
              fill="white"
            />
            <path
              d="M234.754 137.65L313.471 235.304L325.267 249.943L371.922 250L356.172 235.35L251.077 137.65H234.754Z"
              fill="white"
            />
            <path
              d="M127.919 87.8536L108.764 114.609H147.085L127.919 87.8536Z"
              fill="#22675D"
            />
            <path
              d="M157.596 65.168L138.43 91.9115H176.751L157.596 65.168Z"
              fill="#30766C"
            />
            <path
              d="M185.52 43.0557L166.365 69.7992H204.687L185.52 43.0557Z"
              fill="#3A8A7F"
            />
            <path
              d="M214.419 22.1124L195.264 48.856H233.585L214.419 22.1124Z"
              fill="#6CA88B"
            />
            <path d="M242.536 0L223.381 26.7435H261.702L242.536 0Z" fill="#44C486" />
          </svg>
        </div>
      </div>

      <div
        class="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent pointer-events-none"
      />
    </div>
  </div>
</template>
